...
namespace Animation.IK
{
    ...
    /// </summary>
    [Solver2DMenu("Chain With Limits (CCD)")]
    public class CCDSolverWithLimits2D : Solver2D
    {
        ...
        private float[] m_CumulativeRotations;
        ...
        protected override void DoPrepare()
        {
            ...
            if (m_CumulativeRotations == null || m_CumulativeRotations.Length != m_Chain.transformCount)
            {
                m_CumulativeRotations = new float[m_Chain.transformCount];
            }

            for (int i = 0; i < m_Chain.transformCount; ++i)
            {
                m_Positions[i] = m_Chain.transforms[i].position;
                m_CumulativeRotations[i] = m_Chain.transforms[i].localRotation.eulerAngles.z;
            }
        }
        ...
        protected override void DoUpdateIK(List<Vector3> effectorPositions)
        {
            ...
            if (CCD2DLimit.Solve(effectorPosition, GetPlaneRootTransform().forward, iterations, tolerance, Mathf.Lerp(kMinVelocity, kMaxVelocity, m_Velocity), ref m_Positions, ref m_CumulativeRotations, m_limits))
            {
